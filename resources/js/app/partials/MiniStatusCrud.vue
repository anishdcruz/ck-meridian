<template>
  <div class="filterable">
    <x-panel :loading="loading" margin alt>
      ...
      <x-form-group :label="$t('color')" :errors="errors.color">
        <div class="color-input">
          <div class="color-item" v-for="color in colors">
            <div @click="select(color)"
              :class="[`color-box  status-${color}`, form.color === color ? 'color-selected': '']">
              <span>{{color}}</span>
            </div>
          </div>
        </div>
      </x-form-group>
      <x-form-group source="switch" v-model="form.locked" :label="$t('locked')" :errors="errors.locked"></x-form-group>
	  </x-modal>
  </div>
</template>
<script>
