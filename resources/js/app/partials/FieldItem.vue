<template>
    ...
    <div v-if="field.type === 'list'">
      <div class="form-list" v-for="(m, i) in field.list_model">
        <input type="text" class="form-input" v-model="field.list_model[i]">
        <span class="form-list-remove" @click="field.list_model.splice(i, 1)">
            <i class="icon icon-trash-b"></i>
        </span>
      </div>
      <x-button size="sm" icon="plus"
        @click="field.list_model.push('')"></x-button>
    </div>
