	...
	<div class="content-inner">
		<x-panel padding margin>
			...
			<x-row line>
				...
				<x-form-group col="8" :label="$t('invoice_status_on_sent_id')" :errors="errors.invoice_status_on_sent_id">
					<x-tag-input :value="form.invoice_status_on_sent" resource="invoice_statuses" column="name" name="name"
					    @input="value => { form.invoice_status_on_sent_id = value.id; form.invoice_status_on_sent = value }">
					</x-tag-input>
				</x-form-group>
				<x-form-group col="8" :label="$t('invoice_status_on_partially_paid_id')" :errors="errors.invoice_status_on_partially_paid_id">
					<x-tag-input :value="form.invoice_status_on_partially_paid" resource="invoice_statuses" column="name" name="name"
					    @input="value => { form.invoice_status_on_partially_paid_id = value.id; form.invoice_status_on_partially_paid = value }">
					</x-tag-input>
				</x-form-group>
				<x-form-group col="8" :label="$t('invoice_status_on_paid_id')" :errors="errors.invoice_status_on_paid_id">
					<x-tag-input :value="form.invoice_status_on_paid" resource="invoice_statuses" column="name" name="name"
					    @input="value => { form.invoice_status_on_paid_id = value.id; form.invoice_status_on_paid = value }">
					</x-tag-input>
				</x-form-group>
			</x-row>
			<x-row line>
				...
				<x-form-group col="24" :errors="errors.invoice_email_template" :label="$t('invoice_email_template')">
				  	<quill-editor v-model="form.invoice_email_template"></quill-editor>
				</x-form-group>
			</x-row>
		...
		<mini-status-crud url="team-settings/invoice_statuses"
...
<script>
	...
	export default {
		...
		data() {
			return {
				redirect: 'team-settings/invoices',
				form: {
					fields: []
				},
			}
